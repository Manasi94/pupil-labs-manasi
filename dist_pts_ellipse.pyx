...
cimport numpy as np
DTYPE = np.float64
cpdef float dist_pts_ellipse(ellipse, points):
    """
    ...
    cdef float pts =np.array(np.float64(points))
    cdef float rx= dx/2.0
    cdef float ry= dy/2.0
    cdef float angle = (pyangle/180.)*np.pi
    cdef float M_rot,norm_mag,norm_dist,ratio,scaled_error,real_error,error_mag
